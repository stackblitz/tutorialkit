...
export function generateNavigationList(tutorial: Tutorial, baseURL: string): NavList {
  const list: NavList = [];
  const chapterItems = new Map<Chapter['id'], NavItem>();
  const partItems = new Map<Part['id'], NavItem>();
  ...
  for (const lesson of tutorial.lessons) {
    ...
    let partItem = part && partItems.get(part?.id);
    let chapterItem = chapter && chapterItems.get(chapter?.id);
