        ...
        >
          ...
          <AnimatePresence>
            {showDropdown && (
              ...
              >
                <NavListComponent
                  className="space-y-1.5 py-5 pl-5 border-t border-tk-elements-breadcrumbs-dropdown-borderColor overflow-auto max-h-[60dvh]"
                  items={navList}
...
function NavListComponent({
  ...
  return (
    <Accordion.Root asChild collapsible type="single" defaultValue={`${level}-${activeItems[level]}`}>
      <ul className={classNames(className, 'space-y-1.5')}>
...
function NavListItem({ level, type, index, i18n, activeItems, id, title, href, sections }: NavItem & NavListItemProps) {
  ...
  if (!sections) {
    return (
      <li>
        <NavLink href={href} title={title} isActive={isActive} />
      </li>
    );
  }
...
function NavLink({ title, href, isActive }: Pick<NavItem, 'href' | 'title'> & { isActive: boolean }) {
  return (
    <a
      className={classNames(
        'w-full inline-block border border-transparent pr-3 mr-3 transition-theme text-tk-elements-breadcrumbs-dropdown-lessonTextColor hover:text-tk-elements-breadcrumbs-dropdown-lessonTextColorHover px-3 py-1 rounded-1',
        {
          'bg-tk-elements-breadcrumbs-dropdown-lessonBackgroundColor': !isActive,
