...
import themeCSS from '@components/react-examples/theme.css?raw';
...
Those components use TutorialKit's design system which is based on an atomic CSS engine, UnoCSS. You will need a few dependencies in order to use them:
...
* `scope?: string` - Every file paths that does not starts with this scope will be hidden.
...
A component to edit code with syntax highlighting using CodeMirror 6. This component support multi-file edits by caching its state when moving between files.
...
[stackblitz-example]: https://stackblitz.com
