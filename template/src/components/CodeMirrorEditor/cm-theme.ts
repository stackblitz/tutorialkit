...
export function theme(): Extension {
  return [editorTheme, themeIsDark() ? themeSelection.of([getDarkTheme()]) : themeSelection.of([getLightTheme()])];
}

export function reconfigureTheme() {
  return themeSelection.reconfigure(themeIsDark() ? getDarkTheme() : getLightTheme());
}
